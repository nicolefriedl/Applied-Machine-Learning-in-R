...
```{r}
rec_1 <- recipe(survived ~ age + sex + pclass + fare + n_sib_spouse + n_parent_child + embarked, 
                 data = data_trn) |> 
...
```{r}
rda_spec <- discrim_regularized(frac_common_cov = 0, frac_identity = 0)

rda_workflow <- workflow() |> 
  add_recipe(rec_1) |> 
  add_model(rda_spec)

rda_fit <- rda_workflow |> 
  fit(data = data_trn)
```
...
```{r}
rda_pred <- rda_fit |> 
  predict(new_data = data_val)

rda_accuracy <- accuracy_vec(truth = data_val$survived, estimate = rda_pred$.pred_class)

rda_accuracy_df <- tibble(model = "RDA 1", accuracy = rda_accuracy)
error_val <- bind_rows(error_val, rda_accuracy_df)
...
Informed by EDA from your modeling EDA, create a recipe for your first model
```{r}
rec_2 <- recipe(survived ~ age + sex + pclass + fare + n_sib_spouse + n_parent_child + embarked, 
                data = data_trn) |> 
  step_impute_median(age) |>  
  step_dummy(all_nominal(), -all_outcomes()) |>  
...
Fit a `rda` model predicting `survived` from your training feature matrix
```{r}
rda_spec_2 <- discrim_regularized(frac_common_cov = 0, frac_identity = 0)

rda_workflow_2 <- workflow() |> 
  add_recipe(rec_2) |> 
  add_model(rda_spec_2)

rda_fit_2 <- rda_workflow_2 |> 
  fit(data = data_trn)
...
Use `accuracy_vec()` to calculate the validation error (accuracy) of your model. Add this value to your validation error tracking tibble
```{r}
rda_pred_2 <- rda_fit_2 |> 
  predict(new_data = data_val)

rda_accuracy_2 <- accuracy_vec(truth = data_val$survived, estimate = rda_pred_2$.pred_class)

error_val <- error_val |> 
  add_row(model = "RDA 2 (Log Transformation)", accuracy = rda_accuracy_2)
...
```{r}
data_trn <- data_trn |> 
  mutate(age_pclass_interaction = age * pclass)

rec_3 <- recipe(survived ~ age + sex + pclass + fare + n_sib_spouse + n_parent_child + embarked, 
                data = data_trn) |> 
  step_impute_median(age) |>  # Impute missing age values
...
error_val <- error_val |> 
  add_row(model = "RDA with feature engineering", accuracy = rda_accuracy)
